...
class ProductDamageReport extends Component
{
    ...
    public function search()
    {
        ...
        if(@$this->state['start_date']){
            $query->where('damage_date', '>=', $this->state['start_date']);
        }
        if(@$this->state['end_date']){
            $query->where('damage_date', '<=', $this->state['end_date']);
        }
    ...
    {
        return view('livewire.dashboard.reports.product.product-damage-report');
    }
}