<div>
    ...
    <div style="display: flex; justify-content: space-between; align-items:center">
        ...
        <nav aria-label="breadcrumb" style="padding-right: 5px">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><a wire:navigate href="{{ route('purchase-return-details', $purchase_return_id ) }}"
    ...
    <div class="card p-4">
        <div class="row mb-3">
            {{-- <div class="col-auto ">
                <a href='{{ route('purchase-edit', $purchase_return_id) }}' class="btn btn-warning">
                    <i class="fa-solid fa-pen-to-square"></i>
                    Edit
                </a>
            </div> --}}
            <div class="col-auto">
                <a target="_blank" href="{{ route('purchase-invoice', $purchase_return_id) }}" class="btn btn-success">
            ...
            <div class="responsive-table" style="font-size: 0.9em !important;">
                <table class="table table-bordered">
                    <tbody>
                        ...
                        <tr style="font-size: 16px; font-weight: 500">
                            <td>Supplier :</td>
                            <td>{{ $purchase_return_mst->p_name }}</td>
                            <td>Warehouse :</td>
                            <td>{{ $purchase_return_mst->war_name }}</td>
                        </tr>
                        <tr>

                            {{-- <th>Payment Status</th> --}}
                            {{-- <td>{{ $purchase_return_mst->payment_status }}</td> --}}
                        </tr>
                    </tbody>
                ...
                <table class="table table-bordered" style="font-size: 15px; font-weight: 400">
                    ...
                    <tbody>
                        @forelse ($purchase_return_dtl as $purchase_key => $purchase_return)
                        <tr>
                            <td>{{ $purchase_key + 1 }}</td>
                            <td>{{ $purchase_return->item_name }}</td>
                            <td class="text-center">
                                {{ $purchase_return->item_qty }}</td>
                            <td class="text-center">
                                {{ number_format($purchase_return->pr_rate, 2, '.', '') }}
                            </td>
                            <td class="text-center">
                                {{ number_format($purchase_return->discount, 2, '.', '') }}
                            </td>
                            <td class="text-center">
                                {{ number_format($purchase_return->vat_amt, 2, '.', '') }}
                            </td>
                            <td style="text-align: right">
                                {{ number_format($purchase_return->tot_payble_amt, 2, '.', '') }}
                            </td>
                        </tr>
                        @empty
                        <tr>
                    ...
                    <tfoot>
                        <tr style="font-size: 16px; font-weight: 500; text-align: center">
                            ...
                            <td>{{ $purchase_return_mst->total_qty }}</td>
                            <td></td>
                            <td>{{ number_format($purchase_return_mst->tot_discount, 2, '.', '') }}</td>
                            <td>{{ $purchase_return_mst->tot_vat_amt }}</td>
                            <td style="text-align: right">{{ $purchase_return_mst->net_payable_amt }}</td>
                        </tr>
                    </tfoot>
                </table>
                <div class="row">
                                ...
                                <tfoot>
                                    <tr>
                                        ...
                                        <td style="text-align: right; font-size: 15px; font-weight: 500">{{
                                            number_format($purchase_return_mst->tot_paid_amt, 2, '.', '') }}</td>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                    ...
                    <div class="col-md-6">
                        <table class="table table-bordered" style="background: #e4ede6;">
                            <tbody style="font-size: 16px; font-weight: 500;text-align: right">
                                <tr>
                                    <td>Sub Total</td>
                                    <td>{{ number_format($purchase_return_mst->net_payable_amt, 2, '.', '') }}</td>
                                </tr>
                                <tr>
                                    <td>Shipping</td>
                                    <td>{{ number_format($purchase_return_mst->shipping_amt, 2, '.', '') }}</td>
                                </tr>
                                <tr>
                                    <td>Total Amount</td>
                                    <td>{{ number_format($purchase_return_mst->tot_payable_amt, 2, '.', '') }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                        ...
                        <div>
                            <table>
                                <tr style="color: #3C50E0">
                                    <td>Total Amount </td>
                                    <td>&nbsp;:&nbsp;</td>
                                    <td style="text-align: right">{{ number_format($purchase_return_mst->tot_payable_amt, 2,
                                        '.', ',') }}</td>
                                </tr>
                                <tr style="color: darkgreen">
                                    <td>Payment amount </td>
                                    <td>&nbsp;:&nbsp;</td>
                                    <td style="text-align: right">{{ number_format($purchase_return_mst->tot_paid_amt, 2, '.',
                                        ',') }}</td>
                                </tr>
                                <tr style="color: darkred">
                                    <td>Due amount </td>
                                    <td>&nbsp;:&nbsp;</td>
                                    <td style="text-align: right">{{ number_format($purchase_return_mst->tot_due_amt, 2, '.',
                                        ',') }}</td>
                                </tr>
                            </table>
                        </div>
                    ...
                    <div class="col-md-3 mt-3">
                        <div style="">
                            @if ($purchase_return_mst->payment_status == 'RECEIVED')
