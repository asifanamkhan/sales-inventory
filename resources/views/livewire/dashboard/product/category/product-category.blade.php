    ...
    <div style="display: flex; justify-content: space-between; align-items:center">
        ...
        <nav aria-label="breadcrumb" style="padding-right: 5px">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active"><a wire:navigate href="{{ route('product-category') }}"
            ...
            <table class="table table-bordered">
                <thead>
                    <tr>
                        ...
                        <td class="bg-sidebar" >Category name</td>
                        <td class="bg-sidebar text-center" >Action</td>
                    </tr>
                </thead>
                <tbody>
                    @if (count($this->resultCategory) > 0)
                    @foreach ($this->resultCategory as $key => $productCategory)
                    <tr wire:key='{{ $key }}'>
                        <td>{{ $this->resultCategory->firstItem() + $key }}</td>
                        <td>{{ $productCategory->catagories_name }}</td>
                        <td style="display: flex; justify-content:center">
                            <div class="">
                                <button
                                    @click="$dispatch('product-category-edit-modal', {id: {{ $productCategory->tran_mst_id }}})"
