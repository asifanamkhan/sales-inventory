            ...
            <table class="table table-bordered">
                <thead>
                    <tr>
                        ...
                        <td class="bg-sidebar" style="width: 30%">Brand name</td>
                        <td class="bg-sidebar text-center" style="width: 20%">Action</td>
                    </tr>
                </thead>
                <tbody>
                    @if (count($this->resultBrand) > 0)
                    @foreach ($this->resultBrand as $productBrand)
                    <tr>
                        <td>{{ $loop->index + 1 }}</td>
                        <td>{{ $productBrand->brand_name }}</td>
                        <td style="display: flex; justify-content:center">
                            <div class="">
                                <button @click="$dispatch('product-brand-edit-modal', {id: {{ $productBrand->brand_code }}})"
                                    data-toggle="modal" data-target="#{{ $event }}"
